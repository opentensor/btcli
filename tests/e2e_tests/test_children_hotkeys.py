...
def test_set_children_multiple_proportions(local_chain, wallet_setup):
    ...
    if not set_children_result.stdout.strip():
        ...
        pytest.fail(
            f"set_children returned empty stdout. stderr: {set_children_result.stderr}"
        )
...
def test_revoke_children(local_chain, wallet_setup):
    """
    Test revoking all children hotkeys on a subnet.

    Steps:
        1. Create wallets for Alice (parent) and Bob (child)
        2. Create a subnet and register Bob
        3. Start emissions on the subnet
        4. Add stake to Alice's hotkey
        5. Set Bob as a child hotkey with 50% proportion
        6. Revoke all children
        7. Verify the revoke transaction succeeded
    """
    print("Testing revoke_children")
    ...
    # Debug output if parsing fails
    if not revoke_children_result.stdout.strip():
        print(f"stdout is empty")
        print(f"stderr: {revoke_children_result.stderr}")
        print(f"exit_code: {revoke_children_result.exit_code}")
        pytest.fail(
            f"revoke_children returned empty stdout. stderr: {revoke_children_result.stderr}"
        )

    try:
        revoke_output = json.loads(revoke_children_result.stdout)
    except json.JSONDecodeError as e:
        print(f"Failed to parse JSON")
        print(f"stdout: {revoke_children_result.stdout}")
        print(f"stderr: {revoke_children_result.stderr}")
        pytest.fail(
            f"Failed to parse JSON: {e}. stdout: {revoke_children_result.stdout}"
        )

    assert revoke_output[str(netuid)]["success"] is True
    assert isinstance(revoke_output[str(netuid)]["extrinsic_identifier"], str)
    assert revoke_output[str(netuid)]["completion_block"] is not None
    assert revoke_output[str(netuid)]["set_block"] is not None

    print("✅ Passed revoke_children")
