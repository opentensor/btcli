...
def test_unstaking(local_chain, wallet_setup):
    ...
    # Extract extrinsic ID from console output
    # Format: "Your extrinsic has been included as {ext_id}"
    extrinsic_id_pattern = r"Your extrinsic has been included as (\d+-\d+)"
    matches = re.findall(extrinsic_id_pattern, unstake_all.output)
    assert len(matches) == 1, (
        f"Expected exactly one extrinsic ID in output, but found {len(matches)}: {matches}"
    )
    extrinsic_id = matches[0]
    print(f"✅ Verified: unstake_all executed in single extrinsic: {extrinsic_id}")
    
    print("Passed unstaking batching tests 🎉")
