...
def _btcli_completion_var(prog_name: str) -> str:
    return f"_{prog_name.upper().replace('-', '_')}_COMPLETE"


def _append_block_if_missing(file_path: Path, marker: str, block: str) -> bool:
...
def _detect_shell_rc(shell: str) -> Path:
    home = Path(os.path.expanduser("~"))
    if shell == "bash":
        for candidate in (home / ".bashrc", home / ".bash_profile", home / ".profile"):
            if candidate.exists():
                return candidate
        return home / ".bashrc"
    if shell == "zsh":
...
def _default_powershell_profile_path() -> Optional[Path]:
    ...
    # If running on non-Windows, we can't reliably know the user's PowerShell profile.
    return None
...
class CLIManager:
    ...
    def __init__(self):
        ...
        # top-level misc
        self.app.command("completion")(self.completion)
    ...
    def completion(
        ...
        install: bool = typer.Option(
            False,
            "--install",
            help="Install completion for the chosen shell by updating profile files.",
        ),
        rc_path: Optional[Path] = typer.Option(
            None,
            "--rc-path",
            help="Override the profile file to update (bash/zsh) or PowerShell profile path.",
        ),
    ):
        """Print or install shell completion for `btcli`."""

        shell_norm = shell.strip().lower()
        allowed = {"bash", "zsh", "fish", "powershell"}
        if shell_norm not in allowed:
        ...
        # Install paths and blocks
        if shell_norm in {"bash", "zsh"}:
            target_rc = rc_path or _detect_shell_rc(shell_norm)
            marker = f"# >>> btcli completion ({shell_norm}) >>>"
            ...
            changed = _append_block_if_missing(target_rc, marker, block)
            if changed:
                typer.echo(f"Installed completion into: {target_rc}")
                typer.echo("Restart your shell or source the file to activate.")
            else:
                typer.echo(f"Completion already installed in: {target_rc}")
            return

        if shell_norm == "fish":
            completions_dir = Path(os.path.expanduser("~")) / ".config" / "fish" / "completions"
            completions_dir.mkdir(parents=True, exist_ok=True)
            target_file = completions_dir / f"{prog_name}.fish"
            script = get_completion_script(prog_name, complete_var, "fish")
            target_file.write_text(script, encoding="utf-8")
            typer.echo(f"Installed completion into: {target_file}")
            typer.echo("Restart fish to activate (or run `exec fish`).")
            return

        # powershell
        target_profile = rc_path or _default_powershell_profile_path()
        if target_profile is None:
            raise typer.BadParameter(
                "Unable to detect PowerShell profile path. Provide --rc-path to a .ps1 profile file."
            )
        marker = "# >>> btcli completion (powershell) >>>"
        end_marker = "# <<< btcli completion (powershell) <<<"
        block = (
            f"{marker}\n"
        ...
        changed = _append_block_if_missing(target_profile, marker, block)
        if changed:
            typer.echo(f"Installed completion into: {target_profile}")
            typer.echo("Restart PowerShell to activate.")
        else:
            typer.echo(f"Completion already installed in: {target_profile}")
    ...
    def subnets_list(
        ...
        output: Optional[str] = typer.Option(
            ...
            "--output",
            "-o",
            help="Output format: table, json, yaml. Overrides --json-output when set.",
        ),
        columns: Optional[str] = typer.Option(
            ...
            help=(
                ...
                "Available: netuid,name,price,market_cap,emission,tao_flow_ema,liquidity,stake,supply,tempo,mechanisms"
            ),
        ),
        ...
        netuids: str = typer.Option(
            ...
            "--netuids",
            "-n",
            help="Filter to specific netuid(s). Comma-separated, e.g. `--netuids 0,1,2`.",
        ),
        name_contains: Optional[str] = typer.Option(
            None,
            "--name-contains",
            "--name",
            help="Filter subnets whose name contains this string (case-insensitive).",
        ),
        sort_by: Optional[str] = typer.Option(
            None,
            "--sort-by",
            "--sort_by",
            help="Sort by one of: market_cap, netuid, name, price, emission, tao_flow_ema, stake, supply, tempo, mechanisms.",
        ),
        sort_order: Optional[str] = typer.Option(
            None,
            "--sort-order",
        ...
        if output is not None:
            ...
            json_output = output_norm == "json"

        if json_output and live_mode:
            print_error("Cannot use `--json-output` and `--live` at the same time.")
            return

        if output_format in {"json", "yaml"} and live_mode:
            print_error("Cannot use `--output json/yaml` and `--live` at the same time.")
            return
        ...
        if netuids:
            netuids = parse_to_list(
                netuids,
                int,
                "Netuids must be a comma-separated list of ints, e.g., `--netuids 0,1,2`.",
            )
        else:
            netuids = None
        ...
        return self._run_command(
            subnets.subnets_list(
                ...
                limit=limit,
                output=output_format or ("json" if json_output else "table"),
