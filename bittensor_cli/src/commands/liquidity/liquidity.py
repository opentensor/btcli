...
async def add_liquidity_interactive(
    ...
    """
    from bittensor_wallet import Wallet
    from bittensor_cli.src.bittensor.utils import get_hotkey_pub_ss58
    import math
    ...
    try:
        ...
        return False, f"Failed to load wallet: {e}"
    ...
    # Determine hotkey to use (optional for some cases)
    hotkey_ss58 = None
    if wallet_hotkey:
        hotkey_ss58 = get_hotkey_pub_ss58(wallet, wallet_hotkey)
    ...
    if price_low >= current_price.tao:
        ...
        # Ask for hotkey if not provided
        if hotkey_ss58 is None:
            hotkey_input = Prompt.ask(
                f"[{COLORS.G.SUBHEAD_MAIN}]Enter the hotkey to take Alpha stake from (optional, press Enter to skip)[/{COLORS.G.SUBHEAD_MAIN}]",
                default=""
            )
            if hotkey_input:
                hotkey_ss58 = get_hotkey_pub_ss58(wallet, hotkey_input)
            else:
                # Use default hotkey from wallet
        ...
        alpha_balance = Balance.from_tao(alpha_amount)
        ...
        liquidity_to_add = Balance.from_rao(
            int(alpha_balance.rao / (1 / sqrt_price_low - 1 / sqrt_price_high))
        )
    ...
    # Case 2: High price <= current price (only TAO needed)
        ...
        # Ask for hotkey if not provided (has no effect but required for extrinsic)
        if hotkey_ss58 is None:
            hotkey_input = Prompt.ask(
                f"[{COLORS.G.SUBHEAD_MAIN}]Enter the hotkey (optional, press Enter to use default)[/{COLORS.G.SUBHEAD_MAIN}]",
                default=""
            )
            if hotkey_input:
                hotkey_ss58 = get_hotkey_pub_ss58(wallet, hotkey_input)
        ...
        tao_balance = Balance.from_tao(tao_amount)
        ...
        sqrt_price_high = math.sqrt(price_high)
        liquidity_to_add = Balance.from_rao(
            int(tao_balance.rao / (sqrt_price_high - sqrt_price_low))
        )
    ...
    # Case 3: Current price is within range (both TAO and Alpha needed)
        ...
        if hotkey_ss58 is None:
            hotkey_input = Prompt.ask(
                f"[{COLORS.G.SUBHEAD_MAIN}]Enter the hotkey to take Alpha stake from (optional, press Enter to use default)[/{COLORS.G.SUBHEAD_MAIN}]",
                default=""
            )
            if hotkey_input:
                hotkey_ss58 = get_hotkey_pub_ss58(wallet, hotkey_input)
        ...
        # Ask user to enter TAO or Alpha amount
        choice = Prompt.ask(
